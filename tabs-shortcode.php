...
class OLT_Tab_Shortcode {
	...
	public static function tab_shortcode( $atts, $content ) {
		...
		self::$shortcode_data[ self::$current_tab_id ][] = array(
			'title' => $title,
			'id'    => $id,
			'class' => $class,
		);
	...
	public static function tabs_shortcode( $atts, $content ) {
		...
		self::$current_tab_id = 'random-tab-id-' . wp_rand( 0, 1000 );
		...
		?><div id="<?php echo esc_attr( self::$current_tab_id ); ?>" class="<?php echo esc_attr( $shell_class ); ?>">
		<?php
